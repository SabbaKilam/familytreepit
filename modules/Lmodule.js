/**
  ...
  Notes: Now qualifyFunction helper doesn't return true for empty arrays (no vacuous truth)
      ...
      Added L.att which applies a string of comma-seperated attribute assignments; requires L.attribs
*/
...
/*
  L.att depends on L.attribs (above)
*/
L.att = function(attribString){
	const attribArray = attribString.split(`,`);
	for ( let i = attribArray.length; i--; ){
		this.attribs( attribArray[i].trim() );	
	}
};
///////////////////////////////////// 
...
L.attachAllElementsById = function(view){
  ...
  array.forEach.call(allElements, element => {
      if(element.id){
        ...
        element.att = L.att.bind(element); // attach L's att() method here. Only works if L.attribs is attached
...
L.addToView = function( idString, viewObject){
  ...
  element.att = L.att.bind(element);  
...
L.attachNewElement = function(tagname, id, view){
  ...
  try{
    ...
    if(view.toString() === `[object Object]`){
      ...
      newElement.att = L.att.bind(newElement); // attach L's att() method here. Only works if L.attribs is attached      
